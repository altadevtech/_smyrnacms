                  ...
                  <tr key={page.id}>
                    ...
                    <td>
                      <div className="pages-actions">
                        <button onClick={() => toggleStatus(page.id, page.status)} title={page.status === 'published' ? 'Despublicar' : 'Publicar'} className="pages-action-btn">
                          {page.status === 'published' ? <EyeOff size={16} /> : <Eye size={16} />}
                        </button>
                        <Link to={`/admin/pages/${page.id}/versions`} className="pages-action-btn info" title="Histórico de versões">
                          <History size={16} />
                        </Link>
                        <Link to={`/admin/pages/edit/${page.id}`} className="pages-action-btn edit" title="Editar página">
                          <Edit size={16} />
                        </Link>
                        <button onClick={() => handleDelete(page.id)} className="pages-action-btn delete" title="Excluir página">
                          <Trash2 size={16} />
                        </button>
                      </div>
                    </td>
